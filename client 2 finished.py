...
class App(customtkinter.CTk):
    ...
    def receive_messages(self):
        while True:
            try:
                ...
                if payload:
                    self.after(0, lambda p=payload: self.handle_incoming_payload(p))

            except (ConnectionError, BrokenPipeError, ConnectionResetError, OSError, struct.error):
                print("Server disconnected")
                break

    def handle_incoming_payload(self, payload: bytes):
        if payload.startswith(b"TEXT|"):
            text = payload[5:].decode("utf-8", errors="replace")
            self.add_received_message(text)
            return
        ...
        if payload.startswith(b"IMAGE|"):
            try:
                ...
                filename = parts[1].decode("utf-8", errors="replace")
                image_bytes = base64.b64decode(parts[2])
                self.add_received_image(image_bytes, filename)
            except Exception as e:
                print(f"Could not display incoming image: {e}")
            return
        ...
        try:
            ...
            self.add_received_message("[Unsupported message]")
    ...
    def send_message_callback(self):
        ...
        payload = b"TEXT|" + user_text.encode("utf-8")
    ...
    def imageUploader(self):
        ...
        if len(path):
            ...
            payload = b"IMAGE|" + os.path.basename(path).encode("utf-8") + b"|" + base64.b64encode(image_bytes)
            self._send_payload(payload)
    ...
    def __init__(self):
        ...
        try:
            icon_image = Image.open("Messgage chat logo.png")
        ...
        #example users with a Charlie Brown profile picture
        self.add_user_to_sidebar("Joshua", "OIP.webp")
        self.add_user_to_sidebar("Edward", "OIP.webp")
        self.add_user_to_sidebar("Aarvin", "OIP.webp")
        self.add_user_to_sidebar("Austin", "OIP.webp")
        self.add_user_to_sidebar("Rafel", "OIP.webp")
        self.add_user_to_sidebar("Matthew", "OIP.webp")
    ...
    def add_received_image(self, image_bytes, filename):
        try:
            raw_img = Image.open(BytesIO(image_bytes))
            pic = customtkinter.CTkImage(light_image=raw_img, dark_image=raw_img, size=(220, 220))
            ...
            img_bubble.image = pic
            img_bubble.pack(side="top", anchor="w", pady=5, padx=(20, 0))
            self.message_feed._parent_canvas.yview_moveto(1.0)
        except Exception as e:
            print(f"Could not render incoming image: {e}")
            self.add_received_message(f"[Image received: {filename}]")
